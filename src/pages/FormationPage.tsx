...
import { BookOpenCheck, ClipboardCheck, LayoutDashboard, LibraryBig, Settings2, UserPlus, UsersRound } from 'lucide-react'
import { Link, useSearchParams } from 'react-router-dom'
import { AssessmentPlanManager, CurriculumManager, EnrollmentManager, ExpositionManager, GradebookManager, PersonProgressView, StudentsProgressOverview } from '@/features/formation'
import { cn } from '@/lib/utils'

type Section = 'home' | 'students' | 'enrollment' | 'evaluation' | 'curriculum'
...
const SECTIONS: { key: Exclude<Section, 'home'>; label: string; description: string; icon: typeof UsersRound; tone: string }[] = [
  ...
  { key: 'curriculum', label: 'Currículo', description: 'Administrar los manuales y lecciones de la ruta local.', icon: LibraryBig, tone: 'bg-amber-500/10 text-amber-800 dark:text-amber-300' },
]
...
export function FormationPage() {
  ...
  const [section, setSection] = useState<Section>(personId ? 'students' : sectionFromUrl === 'evaluation' || sectionFromUrl === 'enrollment' || sectionFromUrl === 'students' || sectionFromUrl === 'curriculum' ? sectionFromUrl : 'home')
  ...
  return (
    <div className="space-y-6">
      ...
      {section === 'curriculum' && <CurriculumManager />}
