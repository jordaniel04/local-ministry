...
const sections = [
  ...
  { to: '/formation', label: 'Catálogo de formación', description: 'Módulos, ruta local y configuración curricular.', icon: BookOpen, tone: 'bg-violet-500/10 text-violet-700 dark:text-violet-300' },
]
...
export function SettingsPage() {
  return <div className="space-y-6">
    ...
    <div><h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Catálogos y registros</h2><div className="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{sections.map(({ to, label, description, icon: Icon, tone }) => <Link key={to} to={to} className="group rounded-2xl border bg-card p-5 transition hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-sm"><span className={`flex size-10 items-center justify-center rounded-xl ${tone}`}><Icon className="size-5" /></span><h3 className="mt-4 font-semibold">{label}</h3><p className="mt-1 text-sm leading-5 text-muted-foreground">{description}</p><span className="mt-4 block text-sm font-medium text-primary">Abrir →</span></Link>)}<details className="group rounded-2xl border bg-card transition hover:border-primary/50"><summary className="flex min-h-[220px] cursor-pointer list-none flex-col justify-between p-5"><div><span className="flex size-10 items-center justify-center rounded-xl bg-amber-500/10 text-amber-600"><BookOpen className="size-5" /></span><h3 className="mt-4 font-semibold">Ciclos de estudio</h3><p className="mt-1 text-sm leading-5 text-muted-foreground">Administra el ciclo, sus módulos y el módulo activo.</p></div><span className="text-sm font-medium text-primary">Abrir configuración →</span></summary><div className="border-t p-1"><StudyCycleSettings /></div></details></div></div>
  </div>
}
