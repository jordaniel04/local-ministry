import { BarChart3, CalendarRange, UsersRound } from 'lucide-react'
import { Link, useSearchParams } from 'react-router-dom'
import { PersonProgressView, StudentsProgressOverview } from '@/features/formation'
import { QuarterlyReport } from '@/features/reports/QuarterlyReport'

export function ReportsPage() {
  const [searchParams, setSearchParams] = useSearchParams()
  const personId = searchParams.get('person') ?? ''
  const view = searchParams.get('view')
  const showRoute = view === 'route' || Boolean(personId)
  ...
  return <div className="space-y-6">
    ...
    {!showRoute && !showQuarterly ? <section className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <button type="button" onClick={() => setSearchParams({ view: 'quarterly' })} className="group flex min-h-48 flex-col rounded-2xl border bg-card p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <span className="flex size-11 items-center justify-center rounded-xl bg-violet-500/10 text-violet-700 dark:text-violet-300"><CalendarRange className="size-5" /></span>
          <span className="mt-5 font-semibold">Estadística trimestral</span>
          <span className="mt-1 text-sm leading-5 text-muted-foreground">Consulta las cifras del reporte ministerial por trimestre.</span>
          <span className="mt-auto pt-4 text-sm font-medium text-primary">Abrir reporte →</span>
        </button>
      </div>
      ...
      <nav aria-label="Migas de pan" className="flex items-center gap-2 text-sm">
        ...
        {showQuarterly ? <span className="text-muted-foreground">Estadística trimestral</span> : personId ? <><button type="button" onClick={openRoute} className="text-primary hover:underline">Avance en la Ruta</button><span className="text-muted-foreground">/</span><span className="text-muted-foreground">Persona</span></> : <span className="text-muted-foreground">Avance en la Ruta</span>}
      </nav>
      {showQuarterly ? <QuarterlyReport /> : personId ? <>
