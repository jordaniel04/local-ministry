...
export function GradebookManager() {
  ...
  const participants = useMemo(() => attempts.filter((attempt) => attempt.module_id === selectedPlan?.module_id && attempt.status === 'in_progress'), [attempts, selectedPlan?.module_id])
  ...
  useEffect(() => {
    ...
    for (const score of savedScores) next[`${score.item_id}:${score.module_enrollment_id}`] = Number(score.score).toFixed(1)
    setScores(next)
  }, [savedScores])
