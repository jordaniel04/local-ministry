...
import { useActiveStudyCycle } from '../hooks/useStudyCycles'
import {
...
export function ExpositionManager({ mode }: { mode: 'groups' | 'grades' }) {
  ...
  const { data: allExpositions = [] } = useExpositions(moduleId)
  const expositions = allExpositions.filter((item) => item.study_cycle_id === activeCycle?.id)
  ...
  const participants = (currentModuleAttempts.length > 0
    ...
    .filter((attempt) => attempt.study_cycle_id === activeCycle?.id)
  const modulesWithParticipants = useMemo(
    () => modules.filter((module) => attempts.some((attempt) => attempt.study_cycle_id === activeCycle?.id && attempt.module_id === module.id && attempt.status === 'in_progress')),
    [attempts, modules, activeCycle?.id],
  )
  ...
  useEffect(() => {
    ...
    if (!modulesWithParticipants.some((module) => module.id === moduleId)) setModuleId(modulesWithParticipants[0]?.id ?? '')
  ...
  const handleCreateExposition = async (event: React.FormEvent) => {
    ...
    if (!moduleId || !cycleId || !activeCycle || selectedIds.length === 0) return
    const fallbackName = title.trim() || `Exposición ${expositions.length + 1}`
    await createExposition.mutateAsync({
      module_id: moduleId,
      cycle_id: cycleId,
    ...
    setTitle('')
    setPlannedAt('')
    setMessage('Grupo creado. Podrás calificarlo desde Notas de exposición.')
  }
  ...
  const saveEvaluation = async () => {
    ...
    if (selectedExposure.status === 'presented') setEditingEvaluation(false)
  ...
  if (mode === 'grades' && selectedExposure) {
    const isSaving = updateExposition.isPending || saveScore.isPending
    return <section className="space-y-5">
      <button type="button" onClick={() => setSelectedExposureId('')} className="text-sm font-medium text-primary hover:underline">← Volver a los grupos</button>
      <div className="grid gap-5 rounded-2xl border bg-card p-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <div className="mt-5 flex flex-wrap gap-3 lg:col-span-2">
          {locked ? <button type="button" onClick={() => setEditingEvaluation(true)} className="inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium">Editar evaluación</button> : <>
            <button type="button" onClick={() => void saveEvaluation()} disabled={isSaving} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:cursor-wait disabled:opacity-70"><Save className="size-4" /> {isSaving ? 'Guardando...' : 'Guardar avance'}</button>
            <button type="button" onClick={() => void markPresented()} disabled={isSaving} className="inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium disabled:cursor-wait disabled:opacity-70"><Check className="size-4" /> {isSaving ? 'Guardando...' : 'Guardar y marcar presentada'}</button>
          </>}
        </div>
  ...
  return <section className="space-y-5">
    <div className="rounded-2xl border bg-card p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-semibold">{mode === 'groups' ? 'Grupos de exposición' : 'Notas de exposición'}</h2>
          <p className="text-sm text-muted-foreground">{mode === 'groups' ? 'Elige el manual, crea grupos y asigna participantes.' : 'Selecciona un grupo para registrar su nota y observaciones.'}</p>
        </div>
      </div>
      <label className="mt-5 block max-w-xl text-sm font-medium">
        Módulo con participantes
        <select value={moduleId} onChange={(event) => setModuleId(event.target.value)} className="mt-2 w-full rounded-lg border bg-background px-3 py-2">
          {modulesWithParticipants.map((module) => <option key={module.id} value={module.id}>{module.name}</option>)}
        </select>
      </label>
      {modulesWithParticipants.length === 0 && <p className="mt-3 text-sm text-muted-foreground">No hay módulos con participantes en el ciclo activo.</p>}
    </div>
    {mode === 'groups' && (
    <details open className="rounded-2xl border bg-card p-5"><summary className="cursor-pointer font-semibold">Crear grupo de exposición</summary><p className="mt-2 text-sm text-muted-foreground">La exposición pertenece a un ciclo del módulo. Los ciclos se gestionan desde la matrícula; aquí solo se elige el ciclo y sus participantes.</p><form onSubmit={handleCreateExposition} className="mt-4 space-y-4"><div className="grid gap-3 md:grid-cols-3"><select value={cycleId} onChange={(event) => setCycleId(event.target.value)} className="rounded-lg border bg-background px-3 py-2" required><option value="">Seleccionar ciclo del módulo...</option>{cycles.map((cycle) => <option key={cycle.id} value={cycle.id}>{cycle.name}</option>)}</select><input value={title} onChange={(event) => setTitle(event.target.value)} className="rounded-lg border bg-background px-3 py-2" placeholder="Tema o lección (opcional)" /><input type="date" value={plannedAt} onChange={(event) => setPlannedAt(event.target.value)} className="rounded-lg border bg-background px-3 py-2" /></div>{cycles.length === 0 && <p className="text-sm text-amber-700 dark:text-amber-300">Este módulo todavía no tiene ciclos activos. Registra primero el ciclo desde Matrícula.</p>}<div className="max-w-xl"><label className="text-sm font-medium">Agregar participantes</label><div className="mt-2 flex items-center gap-2 rounded-lg border bg-background px-3"><Search className="size-4 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent py-2 outline-none" placeholder="Buscar por nombre..." /></div><div className="mt-2 grid gap-2 sm:grid-cols-2">{visibleParticipants.map((attempt) => <label key={attempt.id} className="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm"><input type="checkbox" checked={selectedIds.includes(attempt.id)} onChange={() => toggleParticipant(attempt.id)} />{personName(attempt.formation_enrollments?.people)}</label>)}</div></div><button type="submit" disabled={!cycleId || selectedIds.length === 0} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50"><Plus className="size-4" /> Crear grupo</button></form></details>
    )}
    {mode === 'groups' && message && <p role="status" className="text-sm text-muted-foreground">{message}</p>}
    <div className="rounded-2xl border bg-card p-5">
      <h3 className="font-semibold">{mode === 'groups' ? 'Grupos creados' : 'Grupos para evaluar'}</h3>
      {expositions.length === 0 ? <p className="mt-4 text-sm text-muted-foreground">Todavía no hay grupos registrados para este módulo.</p> : <div className="mt-3 grid gap-2 md:grid-cols-2">
        {expositions.map((exposition) => mode === 'grades'
          ? <button key={exposition.id} type="button" onClick={() => setSelectedExposureId(exposition.id)} className="rounded-xl border p-3 text-left transition hover:border-primary/50">
              <span className="font-medium">{exposition.group_name}</span>
              <span className="mt-1 block text-xs text-muted-foreground">{exposition.title || 'Sin tema'} · {exposition.status === 'presented' ? 'Presentada' : 'Pendiente'}</span>
              <span className="mt-2 block text-xs font-medium text-primary">Colocar notas →</span>
            </button>
          : <div key={exposition.id} className="rounded-xl border p-3">
              <span className="font-medium">{exposition.group_name}</span>
              <span className="mt-1 block text-xs text-muted-foreground">{exposition.title || 'Sin tema'} · {exposition.status === 'presented' ? 'Presentada' : 'Pendiente'}</span>
            </div>)}
      </div>}
